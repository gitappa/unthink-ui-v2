    ...
    async loadUI(publishedBlogCollection = []) {
        ...
        if (clonedNode) {
            const cards = publishedBlogCollection.map((collection) => {
                ...
                anchor.href = `https://inspiration.samskarahome.com/collections/${collection?.path}?utm_source=unthink&utm_medium=${platform}&utm_campaign=${collection?.collection_id}&utm_content=unthink_collection_featured_widget&unthink_source=unthink_collection_featured_widget&unthink_medium=${platform}&unthink_campaign=${collection?.collection_id}`;
