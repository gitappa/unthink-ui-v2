...
const WishListModal = ({
  ...
  const isLoading =
    ...
    reorderWishlistInProgress ||
    isWishlistFetching ||
    CreatorCollections?.loading; // UPDATE // add remove wishlist as well
  ...
  const getCollectionFilterPayload = useCallback(() => {
    if (featuredFilter === "social_media") {
      return { generated_by: "video_based" };
    }

    if (featuredFilter === "lookbooks") {
      return { generated_by: "lookbook_based" };
    }

    if (featuredFilter === "trending") {
      return { collection_type: "trending" };
    }
  ...
  useEffect(() => {
    if (
      ...
      const payload = getCollectionFilterPayload();
      dispatch(
        getUserCollections({
          user_id: adminUserId,
          product_limits: FETCH_COLLECTIONS_PRODUCT_LIMIT,
  ...
  useEffect(() => {
    if (
      ...
      dispatch(getCreatorCollection(getCollectionFilterPayload()));
    }
  }, [
    isStoreAdminLoggedIn,
    showWishlistModal,
  ...
  return (
    ...
    >
      <div className={styles.drawerBodyContent}>
        <div ref={wishlistBodyRef} className={styles.drawerInner}>
          ...
          {!showCreateOrEditWishlist && !showCollectionDetails && (
            <div>
              <div className={styles.headerContainer}>
                <h1 className={styles.headerTitle}>{WISHLISTS_TITLE}</h1>
                <div>
                  <CloseOutlined
              ...
              {enabledProductToAddInWishlist && collections.length ? (
                <h3 className={styles.selectionMessage}>
              ...
              {isStoreAdminLoggedIn && (
                <>
                  <div className={styles.tabsContainer}>
                    <button
                      onClick={() => handleCollectionClick("Feature")}
                      className={`${styles.tabButtonBase} ${styles.tabButtonLeft} ${activeCollection === "Feature" ? " bg-secondary text-white rounded-l-[10px]" : " border-secondary border-2 text-secondary bg-transparent rounded-l-[10px]  "}`}
                    >
                      Featured Collections
                    </button>
                    <button
                      onClick={() => handleCollectionClick("Creator")}
                      className={`${styles.tabButtonBase} ${styles.tabButtonRight} ${activeCollection === "Creator" ? " bg-secondary text-white rounded-r-[10px]" : "border-secondary border-2 text-secondary bg-transparent rounded-r[10px]"}`}
                    >
                      Creator Collections
                    </button>
                  </div>
                    ...
                    <div className={styles.productSelectionContainer}>
                      {enableSelectProduct ? (
                        ...
                        <Tooltip title="Click and select multiple products to delete">
                          <p
                            className="lg:text-lg text-base text-secondary cursor-pointer"
                  ...
                  {(activeCollection === "Feature" ||
                    ...
                    <div className="flex justify-center mt-3 mb-2 px-4 w-full">
                      <div className="flex w-full">
                        <button
                          onClick={() => onFeaturedFilterChange("social_media")}
                          className={`flex-1 py-1.5 text-sm font-medium border-y border-l border-secondary transition-colors ${featuredFilter === "social_media" ? "bg-secondary text-white" : "text-secondary bg-transparent"} rounded-l-md`}
                        >
                          Social Media
                        </button>
                        <button
                          onClick={() => onFeaturedFilterChange("lookbooks")}
                          className={`flex-1 py-1.5 text-sm font-medium border-y border-l border-secondary transition-colors ${featuredFilter === "lookbooks" ? "bg-secondary text-white" : "text-secondary bg-transparent"}`}
                        >
                          Lookbooks
                        </button>
                        <button
                          onClick={() => onFeaturedFilterChange("trending")}
                          className={`flex-1 py-1.5 text-sm font-medium border border-secondary transition-colors ${featuredFilter === "trending" ? "bg-secondary text-white" : "text-secondary bg-transparent"} rounded-r-md`}
                        >
                          Trending
                        </button>
                      </div>
              ...
              {activeCollection === "Feature" && (
                <div
                  ...
                  id="favorites_collection_container"
                >
                  <Text className={styles.favoritesTitle}>
                    {favorites_collection_name}
                  </Text>
