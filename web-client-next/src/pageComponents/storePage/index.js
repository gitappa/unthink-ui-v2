...
const ReviewCollection = dynamic(
  () => import("../tryForFree/ReviewCollection.js"),
  {
    ssr: false,
    loading: () => (
      <div className={styles.loadingIndicator}>
...
const StorePageWrapper = (props) => {
  ...
  const {
    collection_name: collection_path = router.query.collection_name,
    params: page_params,
  } = props;
  ...
  const [allCollectionData, setAllCollectionData] =
    useState(pageUserCollections); // working
  ...
  const fetchCollectionProduct = useCallback(
    (selectedOption) => {
  ...
  useEffect(() => {
    if (!isRootPage && (isThemePage || isCollectionReviewPage)) {
      // Clear previous collection data before fetching new data
      dispatch(clearInfluencerCollections());
      dispatch(
        getInfluencerCollections({
  ...
  }, [currentPage, isThemePage, isRootPage, isCollectionReviewPage]);
  ...
  }, [isSingleCollectionSharedPage, showWishlistModal, collection_id]);
  ...
  useEffect(() => {
    if (currentPage === 0) {
      ...
      setAllCollectionData((prevCollections) => {
        const newData = pageUserCollections.filter(
          (newItem) =>
  ...
  const onShowMoreClick = () => {
    if (
      authUserCollectionsCount !== 10 &&
      influencerUserCollectionsCount !== 10
    ) {
      return;
    }
    setIsLoading(true);
  ...
  return (
    <div className={styles.storePageContainer}>
      ...
      {showIndividualPageContent ? (
        <>
          ...
          {isCustomProductsPage && (
            <CustomProducts
              isCustomProductsPage={isCustomProductsPage}
              selectedSortOptionProduct={selectedSortOptionProduct}
              handleSortOptionChangeProduct={handleSortOptionChangeProduct}
              {...props}
            />
          )}
      ...
      {isDropDown && (
        <DroppWallet setisDropDown={setisDropDown} isDropDown={isDropDown} />
      )}
